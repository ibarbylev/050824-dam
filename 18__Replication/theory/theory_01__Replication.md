...
- **Состав:** 
    - Состоит из шардов. Каждый шард сам по себе является набором реплик (Replica Set). Это позволяет не только масштабировать данные, но и обеспечивает высокую доступность и отказоустойчивость для каждого шарда.
...
2. Sharded Cluster (Шардированный кластер)
Sharded Cluster - это архитектура MongoDB, предназначенная для горизонтального масштабирования данных, когда данные разделяются на части (шарды) по всему кластеру:

    Шарды (Shards): Каждый шард сам по себе является набором реплик (Replica Set). Это позволяет не только масштабировать данные, но и обеспечивает высокую доступность и отказоустойчивость для каждого шарда.
    Конфигурационные серверы (Config Servers): Хранят метаданные о конфигурации кластера, включая информацию о том, какие данные находятся в каком шарде.
    Маршрутизаторы (Mongos): Эти серверы действуют как интерфейс между клиентским приложением и шардированным кластером, направляя запросы к нужным шардам.
    Асинхронность в шардинге: В контексте шардинга, репликация внутри каждого шарда (который является набором реплик) остается асинхронной, но сам процесс шардинга добавляет уровень сложности, так как данные распределяются и миграция чанков между шардами происходит асинхронно.


Заключение

    Асинхронная репликация предоставляет хорошую производительность за счет возможного временного расхождения данных между узлами, тогда как синхронная репликация (в ограниченном объеме через "w:majority") обеспечивает более высокую консистентность, но может повлиять на скорость операций записи.
    Replica Sets обеспечивают высокую доступность и отказоустойчивость для одного набора данных, а Sharded Clusters масштабируют данные горизонтально, используя репликацию внутри каждого шарда для обеспечения той же высокой доступности.
    Арбитр играет важную роль в наборе реплик для обеспечения кворума, помогая поддерживать работу кластера при сбоях без необходимости хранения данных.


Эти механизмы позволяют MongoDB быть мощным инструментом для работы с большими объемами данных с требованием к высокой доступности и производительности.